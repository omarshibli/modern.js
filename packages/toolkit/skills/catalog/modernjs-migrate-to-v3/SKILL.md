...
- **健壮性（注释/字符串）**：结构定位与结构/标识符信号用「剥离注释**和字符串**」后的文本——注释或普通字符串里的 `defineConfig`/`runtime`/`appTools({ bundler })`/`applyBaseConfig` 不会被误当真实配置或 v2 信号；import 路径信号只从真正的 `import/export-from/dynamic import/require` 模块 specifier 提取（不裸搜字符串）；需要字符串「值」的判断（如 `ssr.mode`）才保留字符串
