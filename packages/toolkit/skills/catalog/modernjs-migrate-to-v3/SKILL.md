...
- **import 路径**：`@modern-js/runtime/bff`→`@modern-js/plugin-bff/runtime`、`@modern-js/runtime/server`→`@modern-js/server-runtime`，并**补充对应依赖**（`@modern-js/plugin-bff` / `@modern-js/server-runtime`；版本**沿用现有 `@modern-js/app-tools`/`runtime` 的协议**——workspace/link/catalog 项目用同协议，否则用目标版本）；命中 BFF 时给 `modern.config` 顶层 plugins **末尾追加** `bffPlugin()`（保持 `[appTools(), bffPlugin()]` 顺序；必要时在 `@modern-js/app-tools` import 上补 `appTools`；无法补则进人工清单，不写半成品）
- **健壮性**：所有结构定位 / 信号判断都先剥离注释（与字符串）再匹配，注释里的 `defineConfig`/`runtime`/`appTools({ bundler })`/`applyBaseConfig` 不会被误当真实配置或 v2 信号
