...
- **健壮性（注释/字符串）**：结构定位（`dev`/`server`/配置对象/括号配平）与结构/标识符信号都用「剥离注释**和字符串**」后的文本——注释或普通字符串里的 `defineConfig`/`runtime`/`appTools({ bundler })`/`applyBaseConfig`/`dev: { port }` 不会被误当真实配置或 v2 信号；import 路径的**检测与改写共用同一套真实 specifier 定位**（`import/export-from/dynamic import/require`），注释/普通字符串里的 `@modern-js/runtime/bff|server` 既不改写也不触发补依赖/插件；tailwind 移除只删真实 import 行与真实 `tailwindcssPlugin()` 调用；需要字符串「值」的判断（如 `ssr.mode`）才保留字符串
